...
async function uploadFileToIPFS (data) {
  ...
  try {
    ...
    const url = `https://gateway.pinata.cloud/ipfs/${responseData.IpfsHash}?filename=${data.originalFilename}`
    return url
  } catch (error) {
    console.log(error)
  }
...
async function uploadJsonToIPFS (json, fileName) {
  try {
    ...
    const url = `https://gateway.pinata.cloud/ipfs/${responseData.IpfsHash}?filename=${fileName}`
    return url
  } catch (error) {
